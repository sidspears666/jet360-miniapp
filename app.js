(() => {
  ...
  const STATUS = {
    pending: { dot: "СЂСџСџРЋ", text: "Teslimat talebi alindi", chip: "chip-pending", badge: "status-pending", next: "accepted", nextLabel: "Kabul ettim" },
    accepted: { dot: "СЂСџСџВ ", text: "Basvuru kabul edildi", chip: "chip-accepted", badge: "status-accepted", next: "completed", nextLabel: "Teslimat tamamlandi" },
    completed: { dot: "СЂСџСџСћ", text: "Teslimat tamamlandi", chip: "chip-completed", badge: "status-completed", next: null, nextLabel: null },
    cancelled: { dot: "СЂСџвЂќТ‘", text: "Siparis iptal edildi", chip: "chip-cancelled", badge: "status-cancelled", next: null, nextLabel: null },
  };
  ...
  function order(raw) {
    ...
    return {
      ...
      customerName: s(raw.customer_name || raw.client_name || raw.driver_name || raw.driverName || raw.contact_name || raw.contactName),
  ...
  function renderOrders() {
    ...
    el.ordersList.innerHTML = items.map((o) => {
      ...
      return `<article class="order-item" data-order-id="${o.id}">
        ...
        <p class="order-meta">СЂСџвЂњРЊ ${esc(o.address || "-")}</p>
        <p class="order-meta">СЂСџРЏв„ў ${esc(o.city || "-")} Р’В· СЂСџРЏВ· ${esc(o.type || "-")}</p>
      </article>`;
    }).join("");
  }
  ...
  function renderHistory() {
    ...
    el.historyList.innerHTML = state.history.slice().sort((a, b) => b.id - a.id).map((o) => {
      ...
      return `<article class="history-item" data-order-id="${o.id}">
        ...
        <p class="history-meta">СЂСџвЂњРЊ ${esc(o.address || "-")}</p>
        <p class="history-meta">СЂСџРЏв„ў ${esc(o.city || "-")} Р’В· СЂСџвЂњвЂ¦ ${esc(fmtDate(o.deliveredAt || o.updatedAt))}</p>
      </article>`;
    }).join("");
  }
  ...
  function renderDetail(o) {
    ...
    if (idx > 0 && status !== "cancelled") btns.push('<button class="btn subtle-back" data-detail-action="ui-back"><span class="back-arrow" aria-hidden="true">←</span> Geri</button>');
