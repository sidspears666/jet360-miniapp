(() => {
  ...
  function toUrlList(v) {
    if (Array.isArray(v)) {
      return v.map(normalizeUrl).filter(Boolean);
    }
    const one = normalizeUrl(v);
    return one ? [one] : [];
  }
  ...
  const DATA_TEMPLATES = toUrlList(
    cfgEndpoints.dataTemplates ||
    cfg.dataTemplates ||
    cfgEndpoints.dataTemplate ||
    cfg.dataTemplate ||
    `${API_BASE}/courier/:telegramId/:section`
  );
  const REGISTER_URLS = toUrlList(
    cfgEndpoints.registerUrls ||
    cfg.registerUrls ||
    cfgEndpoints.registerUrl ||
    cfg.registerUrl ||
    `${API_BASE}/courier/register`
  );
  const ACTION_TEMPLATES = toUrlList(
    cfgEndpoints.actionTemplates ||
    cfg.actionTemplates ||
    cfgEndpoints.actionTemplate ||
    cfg.actionTemplate ||
    `${API_BASE}/order/:orderId/status`
  );
  const AUTO_REFRESH_ENABLED = cfg.autoRefresh !== false;
  const AUTO_REFRESH_MS = Math.max(5000, Number(cfg.autoRefreshMs || 15000));
  const FLOW = ["pending", "accepted", "completed"];
  const STATUS = {
    pending: { dot: "\uD83D\uDFE1", text: "Teslimat talebi alindi", chip: "chip-pending", chipText: "Bekliyor", badge: "status-pending", next: "accepted", nextLabel: "Kabul ettim" },
    accepted: { dot: "\uD83D\uDFE0", text: "Basvuru kabul edildi", chip: "chip-accepted", chipText: "Islemde", badge: "status-accepted", next: "completed", nextLabel: "Teslimat tamamlandi" },
    completed: { dot: "\uD83D\uDFE2", text: "Teslimat tamamlandi", chip: "chip-completed", chipText: "Tamamlandi", badge: "status-completed", next: null, nextLabel: null },
    cancelled: { dot: "\uD83D\uDD34", text: "Siparis iptal edildi", chip: "chip-cancelled", chipText: "Iptal", badge: "status-cancelled", next: null, nextLabel: null },
  };
  ...
  function humanError(e) {
    ...
    if (m.includes("courier_not_linked")) return "Kurye henuz baglanmadi. Numaranizi paylasin.";
    ...
    if (m.includes("contact_phone_not_available")) return "Telegram numarayi Mini App'e vermedi. Lutfen yeniden deneyin.";
  ...
  async function reqAny(method, urls, body) {
    ...
    throw lastErr || err("endpoint_unreachable");
  }

  const api = {
    getOrders: (id) => reqAny("GET", DATA_TEMPLATES.map((t) => applyTemplate(t, { telegramId: id, section: "orders" }))),
    getHistory: (id) => reqAny("GET", DATA_TEMPLATES.map((t) => applyTemplate(t, { telegramId: id, section: "history" }))),
    register: (payload) => reqAny("POST", REGISTER_URLS, payload),
    patchStatus: (id, status) => reqAny("POST", ACTION_TEMPLATES.map((t) => applyTemplate(t, { orderId: id })), { status }),
  };
  ...
  function order(raw) {
    ...
    return {
      ...
      customerName: s(
        ...
        raw.contact_full_name ||
        raw.full_name ||
        raw.bitrix_contact_name
      ),
  ...
  function setTab(tab) {
    if (tab !== "orders" && tab !== "history") tab = "orders";
    state.tab = tab;
    state.detailId = null;
    state.detailUiStatus = null;
    [el.panelOrders, el.panelDetail, el.panelHistory].forEach((x) => x.classList.remove("active"));
  ...
  function openDetail(id) {
    ...
    [el.panelOrders, el.panelHistory].forEach((x) => x.classList.remove("active"));
  ...
  function renderOrders() {
    ...
    el.ordersList.innerHTML = items.map((o) => {
      ...
      return `<article class="order-item" data-order-id="${o.id}">
        <div class="order-top">
          <div class="order-id"><span class="order-dot">${c.dot}</span> No ${o.id}</div>
          <span class="status-chip ${c.chip}">${esc(c.chipText || c.text)}</span>
        </div>
        <p class="order-meta order-meta-line"><span class="meta-icon">&#128205;</span> ${esc(o.address || "-")}</p>
        <div class="order-meta-row">
          <p class="order-meta order-meta-line order-meta-tight"><span class="meta-icon">&#128506;</span> ${esc(o.city || "-")} &nbsp; <span class="meta-icon">&#127991;</span> ${esc(o.type || "-")}</p>
  ...
  function renderHistory() {
    ...
    el.historyList.innerHTML = state.history.slice().sort((a, b) => b.id - a.id).map((o) => {
      ...
      return `<article class="history-item" data-order-id="${o.id}">
        <div class="history-top"><div class="history-id">${c.dot} # ${o.id}</div><span class="status-chip ${c.chip}">${esc(c.chipText || c.text)}</span></div>
        <p class="history-meta">Adres: ${esc(o.address || "-")}</p>
        <p class="history-meta">Sehir: ${esc(o.city || "-")} | Tarih: ${esc(fmtDate(o.deliveredAt || o.updatedAt))}</p>
      </article>`;
    }).join("");
  }
  ...
  function renderDetail(o) {
    ...
    el.detailCustomer.textContent = o.customerName || "-";
    ...
    const btns = [];
    if (c.next) {
      const nextIcon = status === "pending" ? "&#9989;" : "&#128230;";
      btns.push(`<button class="btn primary full" data-detail-action="advance"><span class="btn-emoji">${nextIcon}</span>${esc(c.nextLabel)}</button>`);
    }
    else btns.push('<button class="btn secondary full" disabled>Tamamlandi</button>');
    const idx = FLOW.indexOf(status);
    const openedFromHistory = state.tab === "history";
    if (idx > 0 && status !== "cancelled" && !openedFromHistory) btns.push('<button class="btn subtle-back" data-detail-action="ui-back"><span class="back-arrow" aria-hidden="true">&larr;</span> Geri</button>');
    if (status !== "completed" && status !== "cancelled") btns.push('<button class="btn danger full" data-detail-action="cancel"><span class="btn-emoji">&#128683;</span>Siparisi iptal et</button>');
  ...
  async function requestContact() {
    ...
    return new Promise((resolve, reject) => {
      let done = false;
      let sentDelayTimer = null;
      const scheduleSentResolve = (payload) => {
        if (done) return;
        clearTimeout(sentDelayTimer);
        sentDelayTimer = setTimeout(() => end(payload, false), 1200);
      };
      const normalizeContactResult = (raw) => {
        if (typeof raw === "boolean") return { ok: raw, status: raw ? "sent" : "cancelled" };
        const x = raw && typeof raw === "object" ? raw : {};
        const st = s(x.status).toLowerCase();
        const c = contactData(x);
        const ok = typeof x.ok === "boolean" ? x.ok : (c.ok || st === "sent" || st === "ok" || st === "allowed");
        const status = st || (ok ? "sent" : "cancelled");
        const hasPhone = !!c.phone || !!extractPhoneFromUnknown(x);
        return { ...x, ok, status, hasPhone };
      };
      const onContactRequested = (payload) => {
        const p = payload && typeof payload === "object" ? payload : {};
        const r = normalizeContactResult({ ...p, eventPayload: p });
        if (!r.ok) { end(r, false); return; }
        if (r.hasPhone) { end(r, false); return; }
        scheduleSentResolve(r);
      };
      const end = (v, bad) => {
        if (done) return;
        done = true;
      ...
      try {
        ...
        const maybe = tg.requestContact((r) => {
          const out = normalizeContactResult(r);
          if (!out.ok) { end(out, false); return; }
          if (out.hasPhone) { end(out, false); return; }
          scheduleSentResolve(out);
        });
        if (maybe && typeof maybe.then === "function") {
          ...
          const out = normalizeContactResult(maybe);
          if (!out.ok) { end(out, false); return; }
          if (out.hasPhone) { end(out, false); return; }
          scheduleSentResolve(out);
  ...
  function openSheet(action, id) {
    state.confirmId = Number(id);
    state.confirmAction = s(action);
    if (state.confirmAction === "complete") {
      el.sheetTitle.textContent = "Teslimati tamamladiniz mi?";
      el.sheetText.textContent = "Bu teslimati kesin olarak tamamladiginizdan emin misiniz?";
      el.confirmCancelBtn.classList.remove("danger");
      el.confirmCancelBtn.classList.add("primary");
      el.confirmCancelBtn.textContent = "Evet, tamamlandi";
      el.closeSheetBtn.textContent = "Hayir, geri don";
    } else {
      el.sheetTitle.textContent = "Siparisi iptal et?";
      el.sheetText.textContent = "Bu islem geri alinamaz. Emin misiniz?";
      el.confirmCancelBtn.classList.remove("primary");
      el.confirmCancelBtn.classList.add("danger");
      el.confirmCancelBtn.textContent = "Evet, iptal et";
      el.closeSheetBtn.textContent = "Hayir, geri don";
    }
  ...
  function attach() {
    ...
    el.detailActions.addEventListener("click", async (ev) => {
      ...
      if (act === "ui-back") {
        ...
        if (i > 0) {
          ...
          await commitStatus(o.id, prevStatus);
        }
        return;
      }
      if (act === "cancel") {
        openSheet("cancel", o.id);
      }
    });
    ...
    el.confirmCancelBtn.addEventListener("click", async () => {
      if (!state.confirmId) return;
      const id = state.confirmId;
      const action = state.confirmAction;
      closeSheet();
      if (action === "complete") {
        await commitStatus(id, "completed");
        return;
      }
      await commitStatus(id, "cancelled");
    });

    document.addEventListener("visibilitychange", () => {
      if (!document.hidden) autoRefreshNow();
    });
    window.addEventListener("focus", () => { autoRefreshNow(); });
