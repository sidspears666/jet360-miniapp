(() => {
  ...
  const API_BASE = (window.__JET_CONFIG && window.__JET_CONFIG.apiBase) || "https://flow.gojet.com.tr/webhook";
  ...
  async function req(method, path, body) {
    ...
    const headers = { Accept: "application/json", "X-Telegram-Init-Data": tg ? s(tg.initData) : "" };
    ...
    for (let i = 0; i <= 1; i += 1) {
      ...
      try {
        const r = await fetch(url, { method, headers, body: body == null ? undefined : JSON.stringify(body), signal: ctrl.signal });
  ...
  const api = {
    getOrders: (id) => req("GET", `/courier/${encodeURIComponent(id)}/orders`),
    getHistory: (id) => req("GET", `/courier/${encodeURIComponent(id)}/history`),
    getBalance: (id) => req("GET", `/courier/${encodeURIComponent(id)}/balance`),
    register: (payload) => req("POST", "/courier/register", payload),
    patchStatus: (id, status) => req("PATCH", `/order/${encodeURIComponent(id)}/status`, { status }),
  };
  ...
  async function requestContact() {
    ...
    return new Promise((resolve, reject) => {
      let done = false;
      const end = (v, bad) => { if (done) return; done = true; clearTimeout(timer); bad ? reject(v) : resolve(v); };
      const timer = setTimeout(() => end(err("contact_timeout"), true), 15000);
      try {
        const maybe = tg.requestContact((r) => end(r || {}, false));
        if (maybe && typeof maybe.then === "function") maybe.then((r) => end(r || {}, false)).catch((e) => end(e, true));
        else if (maybe && typeof maybe === "object") end(maybe, false);
      } catch (e) { end(e, true); }
    });
  }
  ...
  function contactData(payload) {
    ...
    const x = p.contact || p.responseUnsafe || p.result || {};
    return {
      phone: phoneFmt(p.phone_number || p.phone || x.phone_number || x.phone || ""),
  ...
  async function register(phone, firstName, lastName) {
    const clean = phoneFmt(phone);
    if (!clean) throw err("phone_required");
    const name = [s(firstName), s(lastName)].filter(Boolean).join(" ").trim() || "Kurye";
    await api.register({ phone: clean, name, telegramId: state.telegramId, username: state.user ? s(state.user.username) : "" });
  }
  ...
  async function doRegisterWithContact() {
    ...
    try {
      ...
      const c = contactData(raw);
      if (!c.phone) throw err("contact_phone_not_received");
  ...
  function attach() {
    ...
    el.mapBtn.addEventListener("click", () => {
      ...
      const o = state.orders.find((x) => x.id === state.detailId) || state.history.find((x) => x.id === state.detailId);
      if (!o) return;
      window.open(`https://maps.google.com/?q=${encodeURIComponent(o.address || "")}`);
    });
  ...
  async function bootstrap() {
    ...
    try {
      ...
      setScreen("welcome");
      if (!isHttpError(e, 404)) toast(humanError(e));
    }
  }
