(() => {
  ...
  function contactData(payload) {
    ...
    const status = s(p.status || x.status || (p.ok === true ? "sent" : "")).toLowerCase();
    const ok = payload === true || p.ok === true || status === "sent" || status === "ok" || status === "allowed";
    return {
      phone: phoneFmt(p.phone_number || p.phone || x.phone_number || x.phone || (state.user && state.user.phone_number) || ""),
  ...
  async function doRegisterWithContact() {
    ...
    try {
      ...
      if (!c.phone && (c.ok || c.status === "sent")) {
        toast("Numara bota gonderildi, kayit kontrol ediliyor...", 2200);
        const linked = await waitRegistrationLink();
        if (linked) {
          renderAll();
          setScreen("app");
          setTab("orders");
