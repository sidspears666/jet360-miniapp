(() => {
  ...
  async function register(phone, firstName, lastName, allowEmptyPhone = false) {
    const clean = phoneDigits(phone);
    if (!clean && !allowEmptyPhone) throw err("phone_required");
  ...
  async function doRegisterWithContact() {
    ...
    try {
      ...
      if (c.ok || c.status === "sent" || c.phone) {
        try {
          await register(c.phone, c.firstName, c.lastName, true);
          preRegistered = true;
        } catch {}
      }
      ...
      if (!preRegistered) await register(c.phone, c.firstName, c.lastName);
      await loadData();
      setScreen("app");
      setTab("orders");
      toast("Kayit tamamlandi.");
  ...
  async function bootstrap() {
    ...
    try {
      ...
      if (!isHttpError(e, 404) && !msg.includes("courier_not_linked")) toast(humanError(e));
    }
  }
